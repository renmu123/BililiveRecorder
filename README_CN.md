...
1. 命令行增加了 `downloader` 子命令用来单独下载 flv 流
2. 最终打包为单个可执行文件进行分发
...
<sup>1</sup>：仅限未经处理的直接从直播服务器下载的原始 FLV 文件。 如果录播是用 FFmpeg 录制的或处理过的就无法修复了，FFmpeg 会进一步损坏有问题的文件。  
...
-   [zyzsdy/biliroku](https://github.com/zyzsdy/biliroku): (大概是)第一个 mikufans 直播录播工具
