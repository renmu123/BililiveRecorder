...
namespace BililiveRecorder.Cli
{
    internal class Program
    {
            ...
            using (var entrypointLogger = BuildLogger(LogEventLevel.Fatal, LogEventLevel.Verbose))
            {
                ...
                try
                {
                    ...
                    var cmd_downloader = new Command("downloader", "Run BililiveRecorder as downloader")
                    {
                        ...
                        new Option<bool>(new []{ "--allow-missing-header" }, () => true, "Allow FLV streams without a header"),
        ...
        private static async Task<int> RunDownloaderModeAsync(DownloaderArguments args)
        {
            ...
            DownloaderConfig config = new DownloaderConfig(args.Url, args.OutputPath, args.Cookie, args.DownloadHeaders, args.MaxSize, args.MaxDuration, args.UseSystemProxy, args.Proxy, args.TimingWatchdogTimeout, args.AllowMissingHeader, args.SplitOnScriptTag, args.DisableSplitOnH264AnnexB);
        ...
        public sealed class DownloaderArguments : SharedArguments
        {
            ...
            public bool AllowMissingHeader { get; set; } = true;
